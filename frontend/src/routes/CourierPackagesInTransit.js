...
import { getInTransitPackages, markPackageStatus, getPackageInfo } from '../services/packageService';
import SortingForm from '../components/SortingForm';
import '../index.css';
import 'bootstrap/dist/css/bootstrap.min.css';
import { Modal, Button } from 'react-bootstrap';

function CourierPackagesInTransit() {
    ...
    const fetchPackages = async () => {
        try {
            const result = await getInTransitPackages(sortOrder, orderBy, pageSize, pageNumber);
    ...
    const handleMarkInTransit = async (packageId) => {
        try {
            await markPackageStatus(packageId, 'Delivered');
            fetchPackages();
        } catch (error) {
                    ...
                    packages.map(pkg => (
                        <div key={pkg.id} className="package-info mb-3">
                            ...
                            <div className="package-tracking">
                                ...
                                <p className='text-black'>Tracking number: {pkg.trackingNumber} <br></br>
...
export default CourierPackagesInTransit;
